...
import { loadWellnessMetrics } from "../store/slices/wellnessSlice";
...
export default function DashboardPage() {
  ...
  const calculateMoodTrends = () => {
    ...
    Object.values(groupedData).forEach(day => {
      ...
      if (count > 0) {
        day.mood_score = day.entries.reduce((sum: number, e: any) => sum + 
          ((e.energy_level + (10 - e.stress_level) + e.focus_level + e.social_desire + e.challenge_seeking) / 5), 0) / count;
        day.energy = day.entries.reduce((sum: number, e: any) => sum + e.energy_level, 0) / count;
        day.stress = day.entries.reduce((sum: number, e: any) => sum + e.stress_level, 0) / count;
        day.focus = day.entries.reduce((sum: number, e: any) => sum + e.focus_level, 0) / count;
      }
    });
  ...
  const calculateStats = () => {
    ...
    const recentMetrics = metrics.filter(metric => 
      new Date(metric.date) >= subDays(new Date(), timeRange === "month" ? 30 : 7)
    );
    ...
    const totalPlayTime = recentMetrics.reduce((sum, metric) => sum + (metric.physical_activity || 0), 0);
    const avgWellnessScore = recentMetrics.length > 0
      ? recentMetrics.reduce((sum, metric) => sum + metric.overall_wellness_score, 0) / recentMetrics.length
      : 0;
    ...
    return {
      ...
      avgWellnessScore: avgWellnessScore.toFixed(1),
      wellnessStreak: user?.wellness_streak || 0
    };
  };
  ...
  const getWellnessInsights = () => {
    ...
    if (parseFloat(stats.avgMoodScore) >= 7.5) {
      insights.push({
        type: "positive",
    ...
    } else if (parseFloat(stats.avgMoodScore) < 5) {
      insights.push({
        type: "attention",
    ...
    if (stats.totalPlayTime > 0 && parseFloat(stats.avgWellnessScore) >= 4) {
      insights.push({
        type: "positive",
                  ...
                  {insights.length > 0 && (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
                      {insights.map((insight, index) => {
                        ...
                        return (
                          <Card key={index} className={`border-2 ${colorClasses[insight.type as keyof typeof colorClasses]}`}>
                    ...
                    <Card className="bg-purple-50 border-0">
                      ...
                      <CardContent>
                        <p className="text-2xl font-bold text-purple-600 mb-2">
                          {Number(stats.avgWellnessScore) > 0 ? `${stats.avgWellnessScore}/10` : 'No data'}
                        </p>
                        <p className="text-sm text-gray-600">
