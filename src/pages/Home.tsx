...
import { createMoodEntry } from "../store/slices/moodSlice";
import { MoodAnalysisResponse, WeatherMood } from "../types";
...
export default function HomePage() {
  ...
  const [textAnalysis, setTextAnalysis] = useState<MoodAnalysisResponse | null>(null);
  ...
  const handleMoodSubmit = async () => {
    ...
    try {
      ...
      const moodEntry = await dispatch(createMoodEntry({
        ...finalMoodValues,
        ...
        mood_analysis: textAnalysis ? {
          ...
          confidence_score: textAnalysis.confidence_score,
          mood_category: textAnalysis.mood_category,
          intensity_level: textAnalysis.intensity_level
        } : undefined,
        context: getTimeContext(),
        weather_mood_factor: getWeatherMoodFactor() as WeatherMood
      })).unwrap();

      navigate(`/recommendations?mood_entry_id=${moodEntry.id}`);
    } catch (error) {
